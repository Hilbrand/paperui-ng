...
class Control {
    ...
    getNode() {
        ...
        return this.parent instanceof Node$1 ? this.parent : this.parent.node;
    }
...
class Node$1 {
    ...
    static fromJSON(json        ) {
        const node = new Node$1(json.name);

        node.id = json.id;
        node.data = json.data;
        node.position = json.position;
        node.name = json.name;
        Node$1.latestId = Math.max(node.id, Node$1.latestId);
...
class Socket$1$1 extends Emitter {
...
class Node$1$1 extends Emitter {
    ...
    bindSocket(el             , type        , io    ) {
        this.sockets.set(io, new Socket$1$1(el, type, io, this.node, this));
    }
...
class EditorView extends Emitter {
    ...
    addNode(node      ) {
        const nodeView = new Node$1$1(node, this.components.get(node.name), this);
    ...
    async fromJSON(json        ) {
        ...
        try {
            await Promise.all(Object.keys(json.nodes).map(async id => {
                ...
                nodes[id] = await component.build(Node$1.fromJSON(node));
...
var actionSocket = new index$2.Socket('Action');
var dataSocket = new index$2.Socket('Data');

class MessageControl extends index$2.Control {
...
class KeydownComponent extends index$2.Component {
  constructor() {
    super('MQTT Receive Trigger');
  }

  builder(node) {
    node.addOutput(new index$2.Output('act', 'Topic name', actionSocket));
    node.addOutput(new index$2.Output('key', 'Payload', dataSocket));
  }
}

class EnterPressComponent extends index$2.Component {
  constructor() {
    super('Enter pressed');
  }
  ...
  builder(node) {
    node
      .addInput(new index$2.Input('act', 'Topic name', actionSocket))
      .addInput(new index$2.Input('key', 'Payload', dataSocket))
      .addOutput(new index$2.Output('then', 'Then', actionSocket))
      .addOutput(new index$2.Output('else', 'Else', actionSocket));
  }
}

class AlertComponent extends index$2.Component {
  constructor() {
    super('Alert');
  }
  ...
  builder(node) {
    ...
    node
      .addControl(ctrl)
      .addInput(new index$2.Input('act', '', actionSocket));
  }
}
...
class OhRuleEditor extends HTMLElement {
    ...
    connectedCallback() {
        var editor = new index$2.NodeEditor('tasksample@0.1.0', this);
